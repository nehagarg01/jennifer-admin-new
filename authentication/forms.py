...
class RequestResetPasswordForm(forms.Form):
    ...
    def __init__(self, local_host=None, data=None):
        ...
        super(RequestResetPasswordForm, self).__init__(data)

    def send(self):
        email = self.cleaned_data['email']
        request = self.create_request_instance()
        reset_url = self.local_host + request.get_reset_password_url()
...
class RequestEmailChangeForm(RequestResetPasswordForm):
